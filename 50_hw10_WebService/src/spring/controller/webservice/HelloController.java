package spring.controller.webservice;
...
@Controller("Spring.webservice.HelloController")
...
public class HelloController {
	...
	public ModelAndView hello() {
		ApplicationContext context = new ClassPathXmlApplicationContext("spring/controller/webservice/spring.xml");
	...
	public ModelAndView doHello(@Valid HelloModel helloModel, BindingResult bindingResult){		
		...
		try {
			...
			try{
				feeModel = restTemplate.postForObject(new StringBuilder(fee_service).append("/findByName").toString(), feeModel, FeeModel.class);
				count = feeModel.getCount();
				
				if (count >= 0) {
					feeModel.setCount(++count);
					
					restTemplate.postForObject(new StringBuilder(fee_service).append("/update").toString(), feeModel, FeeModel.class);
				}
			...
			restTemplate.postForObject(new StringBuilder(fee_service).append("/create").toString(), feeModel, FeeModel.class);
